...
function ParkingBookingForm() {
    ...
    useEffect(() => {
        const fetchLocations = async () => {
            ...
            setError(null);
            try {
                const res = await fetch(`https://${IP}/malls/locations`, {
                    headers: new Headers({
    ...
    useEffect(() => {
        const fetchMalls = async () => {
            ...
            setError(null);
            try {
                const res = await fetch(`https://${IP}/malls?location=${tempData.location}`, {
                    headers: new Headers({
                        "ngrok-skip-browser-warning": "69420",
                    }),
                });
    ...
    useEffect(() => {
        const fetchFloors = async () => {
            ...
            setError(null);
            try {
                const res = await fetch(`https://${IP}/slots/floors?mall_id=${tempData.mall}`, {
                    headers: new Headers({
                        "ngrok-skip-browser-warning": "69420",
                    }),
                });
    ...
    useEffect(() => {
        const fetchSlots = async () => {
            ...
            setError(null);
            try {
                const res = await fetch(`https://${IP}/slots?mall_id=${tempData.mall}&floor_no=${tempData.floor}`, {
                    headers: new Headers({
                        "ngrok-skip-browser-warning": "69420",
                    }),
                });
    ...
    useEffect(() => {
        ...
        if (!activeBooking) {
            ...
            fetch(`https://${IP}/book?slot_id=${activeBooking}`, {
                headers: new Headers({
                    "ngrok-skip-browser-warning": "69420",
                }),
            })
    ...
    const isStepValid = () => {
        if (step === 1) return formData.name && formData.phone.length === 10 && formData.vehicle_no;
        if (step === 2) return tempData.location && tempData.mall;
        if (step === 3) return formData.slot;
        return true;
    };

    const handleSlotSelection = (slot) => slot.is_available === 1 && setFormData({ ...formData, slot: slot.id });

    const handleSubmit = (e) => {
        e.preventDefault();
        fetch(`https://${IP}/users`, {
            method: 'POST',
            headers: {
        ...
        fetch(`https://${IP}/slots`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', "ngrok-skip-browser-warning": "69420" },
            body: JSON.stringify({ id: formData.slot, is_available: 2 }),
        });
    ...
    return (
        <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
            <div className="w-full max-w-2xl bg-gray-800 text-white rounded-xl shadow-lg p-8 transition-all duration-300">
                <img src='../images/logo.jpeg' alt='logo' className='w-40 place-self-center'></img>
                <h1 className="text-3xl font-semibold mb-6 text-center pt-5">Parking Slot Booking</h1>
                            ...
                            {slots.map((slot) => (
                                <div
                                    ...
                                    className={`p-4 text-center rounded-lg cursor-pointer
                                        ${slot.is_available === 0 ? 'bg-red-500' : ''}
                                        ${slot.is_available === 1 ? 'bg-green-500 hover:bg-green-600' : ''}
                                        ${slot.is_available === 2 ? 'bg-yellow-500' : ''}
