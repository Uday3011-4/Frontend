...
function RegisterForm({ onToggle }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await fetch(`https://${IP}/admin/register`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', "ngrok-skip-browser-warning": "69420" },
...
function Dashboard({ mallId }) {
    ...
    useEffect(() => {
        const fetchFloors = async () => {
            ...
            try {
                const res = await fetch(`https://${IP}/slots/floors?mall_id=${tempData.mall}`, { headers: { "ngrok-skip-browser-warning": "69420" } });
    ...
    useEffect(() => {
        const fetchSlots = async () => {
            ...
            try {
                const res = await fetch(`https://${IP}/slots?mall_id=${tempData.mall}&floor_no=${tempData.floor}`, { headers: { "ngrok-skip-browser-warning": "69420" } });
    ...
    const handleSlotSelection = (slot) => {
        if (!slot.is_available) {
            fetch(`https://${IP}/slots/admin?slot_id=${slot.id}`, { headers: { "ngrok-skip-browser-warning": "69420" } })
                .then((res) => res.json())
                .then((data) => {
...
function AdminDashboard() {
    ...
    const handleLoginSuccess = async (id) => {
        setAdminId(id);
        fetch(`https://${IP}/malls/admin?id=${id}`, { headers: { "ngrok-skip-browser-warning": "69420" } })
            .then((res) => res.json())
            .then((data) => {
                if (data.length > 0) {
                    setMallId(data[0].id);
